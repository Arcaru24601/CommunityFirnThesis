# -*- coding: utf-8 -*-
"""
Created on Thu Feb  9 15:37:47 2023
...
class CoD_plotter():
    ...
    def __init__(self,j,filepath=None,Models=None,KtC=False):
        ...
        self.KtC = KtC
        self.CoD = np.array([78,78,80,100,123])
        return
    def CoD_out(self):
...
x = ['NGRIP', 'NEEM', 'Summit','DomeC','South']
x2 = Models
...
Sit = ['NGRIP', 'NEEM', 'Summit','DomeC','South']


CoD_out = np.zeros((5,13))
CoD_diff = np.zeros((5,13))
...
Temp = np.array([242.05,242.05,241.75,219.15,222.15]).reshape((5,1))
Acc = np.array([0.19,0.22,0.23,0.027,0.08]).reshape((5,1))
CoD = np.array([78,78,80,100,123]).reshape((5,1))
Site = ['NGRIP', 'NEEM', 'Summit','DomeC','South']
header = ['Temp','Acc','CoD', 'HLD', 'HLS','Li4','Li1','HEL','ArtS','ArtT','GOU','BAR','MOR','KM','CRO','LIG'] 
...
with open('CoDKindler.tex', 'w') as tf:
     tf.write(df.style.to_latex(column_format="cccccc", position="h", position_float="centering",
                hrules=True, label="table:5", caption="Close-off-depth for Buizert",
...
with open('CoDdiffKindler.tex', 'w') as tf:
     tf.write(df2.style.to_latex(column_format="cccccc", position="h", position_float="centering",
                hrules=True, label="table:5", caption="Close-off-depth diff for Buizert",
                multirow_align="t", multicol_align="r")  
              )
