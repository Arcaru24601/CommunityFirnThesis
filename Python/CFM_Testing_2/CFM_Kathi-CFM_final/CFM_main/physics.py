...
class FirnPhysics:
    ...
    def HL_dynamic(self):
        ...
        A_instant = self.bdotSec[
                        self.iii] * self.steps * S_PER_YEAR * RHO_I_MGM  # Accumulation in units m W.E. per year
        ...
        if self.bdot_type == 'instant':
            ...
            if (self.FirnAir and self.AirRunType == 'steady'):
                ...
                drho_dt[self.rho < RHO_1] = k1 * np.exp(-Q1 / (R * Tcon[self.rho < RHO_1])) * (
                            RHO_I_MGM - self.rho[self.rho < RHO_1] / 1000) * A_instant ** aHL * 1000 / S_PER_YEAR
                drho_dt[self.rho >= RHO_1] = k2 * np.exp(-Q2 / (R * Tcon[self.rho >= RHO_1])) * (
                ...
                viscosity[self.rho < RHO_1] = (self.rho[self.rho < RHO_1] * self.sigma[self.rho < RHO_1]) / (2) / \
                                              drho_dt[self.rho < RHO_1]
                viscosity[self.rho >= RHO_1] = (self.rho[self.rho >= RHO_1] * self.sigma[self.rho >= RHO_1]) / (2) / \
                                               drho_dt[self.rho >= RHO_1]
            ...
            drho_dt[self.rho < RHO_1] = k1 * np.exp(-Q1 / (R * self.Tz[self.rho < RHO_1])) * (
                        RHO_I_MGM - self.rho[self.rho < RHO_1] / 1000) * (
                                        A_mean[self.rho < RHO_1]) ** aHL * 1000 / S_PER_YEAR
            drho_dt[self.rho >= RHO_1] = k2 * np.exp(-Q2 / (R * self.Tz[self.rho >= RHO_1])) * (
                        RHO_I_MGM - self.rho[self.rho >= RHO_1] / 1000) * (
    ...
    def HL_Sigfus(self):
        ...
        sigma550 = ((self.rho[z2first] - RHO_1) * self.sigma[z2first - 1] + (RHO_1 - self.rho[z2first - 1]) *
                    self.sigma[z2first]) / (self.rho[z2first] - self.rho[
            z2first - 1])  # Direct calculation faster than interpolation.
        ...
        if self.bdot_type == 'instant':
            drho_dt[z1mask] = k1 * np.exp(-Q1 / (R * self.Tz[z1mask])) * (
                        RHO_I_MGM - self.rho[z1mask] / 1000) * A_instant ** aHL * 1000 / S_PER_YEAR
            drho_dt[z12mask] = k2 * np.exp(-Q2 / (R * self.Tz[z12mask])) * (
                        RHO_I_MGM - self.rho[z12mask] / 1000) * A_instant ** bHL * 1000 / S_PER_YEAR
        elif self.bdot_type == 'mean':
            drho_dt[z1mask] = k1 * np.exp(-Q1 / (R * self.Tz[z1mask])) * (RHO_I_MGM - self.rho[z1mask] / 1000) * (
            A_mean[z1mask]) ** aHL * 1000 / S_PER_YEAR
            drho_dt[z12mask] = k2 * np.exp(-Q2 / (R * self.Tz[z12mask])) * (RHO_I_MGM - self.rho[z12mask] / 1000) * (
            A_mean[z12mask]) ** bHL * 1000 / S_PER_YEAR

        drho_dt[z2mask] = kSig * (sigmaDiff * rhoDiff[z2mask]) / (
                    GRAVITY * np.log((RHO_I_MGM - RHO_1 / 1000) / (rhoDiff[z2mask])))
        drho_dt[zImask] = 0
    ...
    def Li_2004(self):
        ...
        if self.bdot_type == 'instant':
            ...
            dr_dt = (RHO_I - self.rho) * A_instant * (139.21 - 0.542 * self.T_mean[self.iii]) * 8.36 * (
                        K_TO_C - self.Tz) ** -2.061
        elif self.bdot_type == 'mean':
            dr_dt = (RHO_I - self.rho) * A_mean * (139.21 - 0.542 * self.T_mean[self.iii]) * 8.36 * (
                        K_TO_C - self.Tz) ** -2.061
    ...
    def Li_2011(self):
        ...
        if self.bdot_type == 'instant':
            ...
            dr_dt[self.rho <= RHO_1] = (RHO_I - self.rho[self.rho <= RHO_1]) * A_instant * beta1 * 8.36 * (
                        K_TO_C - self.Tz[self.rho <= RHO_1]) ** -2.061
            dr_dt[self.rho > RHO_1] = (RHO_I - self.rho[self.rho > RHO_1]) * A_instant * beta2 * 8.36 * (
                        K_TO_C - self.Tz[self.rho > RHO_1]) ** -2.061
            ...
            dr_dt[self.rho <= RHO_1] = (RHO_I - self.rho[self.rho <= RHO_1]) * A_mean[self.rho <= RHO_1] * beta1[
                self.rho <= RHO_1] * 8.36 * (K_TO_C - self.Tz[self.rho <= RHO_1]) ** -2.061
            dr_dt[self.rho > RHO_1] = (RHO_I - self.rho[self.rho > RHO_1]) * A_mean[self.rho > RHO_1] * beta2[
                self.rho > RHO_1] * 8.36 * (K_TO_C - self.Tz[self.rho > RHO_1]) ** -2.061
    ...
    def Arthern_2010S(self):
        ...
        if self.bdot_type == 'instant':
            ...
            dr_dt[self.rho < RHO_1] = (RHO_I - self.rho[self.rho < RHO_1]) * ar1 * A_instant * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            dr_dt[self.rho >= RHO_1] = (RHO_I - self.rho[self.rho >= RHO_1]) * ar2 * A_instant * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))
        elif self.bdot_type == 'mean':
            dr_dt[self.rho < RHO_1] = (RHO_I - self.rho[self.rho < RHO_1]) * ar1 * A_mean_1 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            dr_dt[self.rho >= RHO_1] = (RHO_I - self.rho[self.rho >= RHO_1]) * ar2 * A_mean_2 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))

        drho_dt = dr_dt / S_PER_YEAR

        viscosity[self.rho < RHO_1] = (self.rho[self.rho < RHO_1] / (2 * self.sigma[self.rho < RHO_1])) / drho_dt[
            self.rho < RHO_1]
        viscosity[self.rho >= RHO_1] = (self.rho[self.rho >= RHO_1] / (2 * self.sigma[self.rho >= RHO_1])) / drho_dt[
            self.rho >= RHO_1]
    ...
    def Arthern_2010T(self):
        ...
        drho_dt[self.rho < RHO_1] = kc1 * (RHO_I - self.rho[self.rho < RHO_1]) * np.exp(
            -Ec / (R * self.Tz[self.rho < RHO_1])) * self.sigma[self.rho < RHO_1] / (self.r2[self.rho < RHO_1])
        drho_dt[self.rho >= RHO_1] = kc2 * (RHO_I - self.rho[self.rho >= RHO_1]) * np.exp(
            -Ec / (R * self.Tz[self.rho >= RHO_1])) * self.sigma[self.rho >= RHO_1] / (self.r2[self.rho >= RHO_1])
    ...
    def Helsen_2008(self):
        ...
        if self.bdot_type == 'instant':
            ...
            dr_dt = (RHO_I - self.rho) * A_instant * (76.138 - 0.28965 * self.T_mean[self.iii]) * 8.36 * (
                        K_TO_C - self.Tz) ** -2.061
        elif self.bdot_type == 'mean':
            dr_dt = (RHO_I - self.rho) * A_mean * (76.138 - 0.28965 * self.T_mean[self.iii]) * 8.36 * (
                        K_TO_C - self.Tz) ** -2.061
    ...
    def Simonsen_2013(self):
        ...
        if self.bdot_type == 'instant':
            ...
            dr_dt[self.rho < RHO_1] = F0 * (RHO_I - self.rho[self.rho < RHO_1]) * ar1 * A_instant * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            dr_dt[self.rho >= RHO_1] = F1 * gamma * (
                        RHO_I - self.rho[self.rho >= RHO_1]) * ar2 * A_instant * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))
        elif self.bdot_type == 'mean':
            gamma = 61.7 / (A_mean_2 ** (0.5)) * np.exp(-3800.0 / (R * self.T_mean[self.iii]))
            dr_dt[self.rho < RHO_1] = F0 * (RHO_I - self.rho[self.rho < RHO_1]) * ar1 * A_mean_1 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            dr_dt[self.rho >= RHO_1] = F1 * gamma * (
                        RHO_I - self.rho[self.rho >= RHO_1]) * ar2 * A_mean_2 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))
    ...
    def Ligtenberg_2011(self):
        ...
        if self.bdot_type == 'instant':
            ...
            M_0 = np.max((0.25, M_0))
            M_1 = np.max((0.25, M_1))
            dr_dt[self.rho < RHO_1] = (RHO_I - self.rho[self.rho < RHO_1]) * M_0 * ar1 * A_instant * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            dr_dt[self.rho >= RHO_1] = (RHO_I - self.rho[self.rho >= RHO_1]) * M_1 * ar2 * A_instant * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))
        elif self.bdot_type == 'mean':
            ...
            M_0[M_0 < 0.25] = 0.25
            M_1[M_1 < 0.25] = 0.25
            dr_dt[self.rho < RHO_1] = (RHO_I - self.rho[self.rho < RHO_1]) * M_0 * ar1 * A_mean_1 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            dr_dt[self.rho >= RHO_1] = (RHO_I - self.rho[self.rho >= RHO_1]) * M_1 * ar2 * A_mean_2 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))
    ...
    def Goujon_2003(self):
        ...
        D0 = 0.00226 * self.T_mean[
            self.iii] + 0.03  # D0 is the relative density of transition from zone 1 to 2. Here is from Arnaud et al. (2000) eq. 8, not Goujon (Goujon uses C, not K)
        ...
        A = 7.89e3 * np.exp(
            -Qgj / (R * self.Tz)) * 1.0e-3  # A given in MPa^-3 s^-1, Goujon uses bar as pressure unit. Eq. A5 in Goujon
    ...
    def Breant2017(self):
        ...
        Q2 = 75000
        Q3 = 1500
        A = A0 * (a1 * np.exp(-Q1 / (R * self.Tz)) + a2 * np.exp(-Q2 / (R * self.Tz)) + a1 * np.exp(
            -Q3 / (R * self.Tz)))
    ...
    def Crocus(self):
        ...
        if CHybrid:
            ...
            dr_dt[self.rho >= RHO_CC] = ((RHO_I - self.rho[
                self.rho >= RHO_CC]) * M_1 * ar2 * A_mean_2 * GRAVITY * np.exp(
                -Ec / (R * self.Tz[self.rho >= RHO_CC]) + Eg / (R * self.T_mean[self.iii]))) / S_PER_YEAR
            drho_dt[self.rho >= RHO_I] = 0
    ...
    def GSFC2020(self):
        ...
        if self.bdot_type == 'instant':
            ...
            c1 = ar1 * A_mean_1 ** alpha1 * GRAVITY * np.exp(
                -Ec1 / (R * self.Tz[self.rho < RHO_1]) + Eg / (R * self.T_mean[self.iii]))
            c2 = ar2 * A_mean_2 ** alpha2 * GRAVITY * np.exp(
                -Ec2 / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))
    ...
    def Max2018b(self):
        ...
        msk2 = self.rho >= RHO_1
        dr_dt[msk] = k0 * np.exp(-1 * Q / (R * self.Tz[msk])) * (RHO_I - self.rho[msk]) * self.sigma[msk] / self.age[
            msk]
    ...
    def Max2018(self):
        ...
        dr_dt[self.rho >= RHO_1] = ((RHO_I - self.rho[self.rho >= RHO_1]) * M_1 * ar2 * A_mean_2 * GRAVITY * np.exp(
            -Ec / (R * self.Tz[self.rho >= RHO_1]) + Eg / (R * self.T_mean[self.iii]))) / S_PER_YEAR
    ...
    def graincalc(self, iii):
        ...
        if self.MELT:
            ...
            sat[np.where(porespace > 0)[0]] = self.LWC[np.where(porespace > 0)[0]] / porespace[
                np.where(porespace > 0)[0]]  # VV
