...
def thetaeff_equaliser(th_i2, th_s2, LWC2, dz2):
    ...
    lwflux = ((dz2[0] * (th_s2[0] - th_i2[0])) ** (-1) + (dz2[1] * (th_s2[1] - th_i2[1])) ** (-1)) ** (-1) * (
                th_e0[0] - th_e0[1])
    return (lwflux)
...
def dfdg_derivative(th_sfull, th_ifull, th_efull, alphafull, nfull, mfull, dzfull):
    ...
    dfdg = 1 / ((th_s - th_i) * alpha * n * m * dz) * th_e ** (-1 * (1 + 1 / m)) * (th_e ** (-1 / m) - 1) ** (
                (1 - n) / n) + \
           1 / ((th_sd - th_id) * alphad * nd * md * dzd) * th_ed ** (-1 * (1 + 1 / md)) * (th_ed ** (-1 / md) - 1) ** (
                       (1 - nd) / nd)
    return (dfdg)
