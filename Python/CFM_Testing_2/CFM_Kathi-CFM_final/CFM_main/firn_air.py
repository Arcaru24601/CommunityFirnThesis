...
class FirnAir:
    ...
    def porosity(self):  # ,rho,T
        ...
        if self.cg['runtype'] == 'steady':
            self.bcoRho = 1 / (
                        1 / (RHO_I) + self.cg['steady_T'] * 6.95E-7 - 4.3e-5)  # Martinerie mean close-off density
            # self.bcoRho = 825.3 
        else:
        ...
        self.por_cl[self.por_tot > 0] = alpha * self.por_tot[self.por_tot > 0] * (
                    self.por_tot[self.por_tot > 0] / self.por_co) ** (-7.6)
    ...
    def firn_air_diffusion(self, AirParams, iii):
        ...
        self.air_pressure = (self.p_a * np.exp(M_AIR * GRAVITY * self.z / (R * self.Tz))) * volfrac - (
                    self.p_a * np.exp(
                M_AIR * GRAVITY * self.z / (R * self.Tz)))  # assume air pressure is atmos in entire column
        ...
        airdict = {
            ...
            'dz': self.dz,
            'rho': self.rho,
            'gravity': self.cg['gravity'],  # TODO: here we get thermal and gravitational contribution!!! --> solver.py!
