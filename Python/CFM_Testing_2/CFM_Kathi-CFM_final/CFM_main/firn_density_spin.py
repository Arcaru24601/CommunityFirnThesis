...
accumulation rate, and surface. This works well for long model runs
...
class FirnDensitySpin:
    ...
    def __init__(self, config, climateTS=None):
        ...
        Sets up the initial spatial grid, time grid, accumulation rate, age, density, mass, stress, and temperature of
        the model run
        ...
        # 'merging': If a model volume gets too thin, merge it with another. Needed for numerical stability with melt
        # schemes.
        ...
        if self.c['doublegrid']:
            ...
            # adjust ice equivalent thickness of nodes to a thickness approximated by HL analytic
            dznew = 917 / self.rho * self.dz
        ...
        dt1 = S_PER_YEAR / self.c['stpsPerYear']  # number of seconds per step
        self.stp = int(
            self.years * S_PER_YEAR / dt1)  # number of steps for the whole spinup time (in seconds)- total steps
        self.t = 1.0 / self.c['stpsPerYear']  # years per time step
        # array of length of number of total steps, each entry containing the number of seconds per step
        ...
        self.Ts = self.temp0 * np.ones(self.stp)  # array of length of number of time steps filled with Ts

        if self.c['SeasonalTcycle']:  # impose seasonal temperature cycle of amplitude 'TAmp'
            if self.c['SeasonalThemi'] == 'north':  # This is for Greenland
        ...
        if self.c['ReehCorrectedT']:  # temperature correction accounting for latent heat, Reeh 1991 (5) Reeh 2008 (16)
            self.Tz = self.Tz + 26.6 * self.SIR
        ...
        if self.c['strain']:  # (strain: Belastung/Spannung/Zerrung)
            self.du_dx = np.zeros(self.gridLen)  # du_dx is the horizontal strain rate
    ...
    def time_evolve(self):
        ...
        based on the user specified number of timesteps in the model run. Updates the firn density using a user
        specified
        """
        self.steps = 1 / self.t  # this is time steps per year; self.t = 1.0/self.c['stpsPerYear'] (years per time step)
        ...
        for iii in range(self.stp):  # remember: self.stp is the number of total time steps
            ...
            PhysParams = {
                ...
                'bdotSec': self.bdotSec,  # vector of accumulation at each time step
                'bdot_mean': self.bdot_mean,  # = np.ones_like(self.dz) * self.bdot0
                'bdot_type': self.c['bdot_type'],  # 'instant' or 'mean'
                'Tz': self.Tz,
                'T_mean': self.T_mean,
                'T10m': self.T10m,
                'T50': self.T50,
                'rho': self.rho,  # density profile (initially from Herron & Langway)
                'mass': self.mass,  # self.dz * self.rho
                'sigma': self.sigma,
                'dt': self.dt[iii],
                'Ts': self.Ts,
                'r2': self.r2,  # related to surface grain size 'r2s0'
                'age': self.age,  # age profile
                'physGrain': self.c['physGrain'],
                'calcGrainSize': self.c['calcGrainSize'],
                'r2s0': self.c['r2s0'],  # surface grain size at each time step
                'GrGrowPhysics': self.c['GrGrowPhysics'],
                'z': self.z,
                'rhos0': self.rhos0[iii],  # surface density at each time step
                'dz': self.dz,
                'LWC': self.LWC,
                'MELT': self.MELT,
            ...
            # choose densification-physics based on user input --> physics.py
            physicsd = {
            ...
            # following Reeh 1991 parameterization
            ...
            self.bdot_mean = (np.concatenate(
                ([self.mass_sum[0] / (RHO_I * S_PER_YEAR)], self.mass_sum[1:] * self.t / (self.age[1:] * RHO_I)))) * \
                             self.c['stpsPerYear'] * S_PER_YEAR

            # Update grain growth #VV
            # VV calculate this before accumulation (because the new surface layer should not be subject to grain
            # growth yet
            if self.c['physGrain']:
