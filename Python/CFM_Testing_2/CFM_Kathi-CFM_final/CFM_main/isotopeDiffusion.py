    ...
    def __init__(self, spin, config, isotope, stp, z, modeltime=None):
        ...
        try:
            ...
            if spin:
                ...
                del_z_init = read_init(self.c['resultsFolder'], self.c['spinFileName'],
                                       'IsoSpin_{}'.format(self.isotope))
                iso_sig2_init = read_init(self.c['resultsFolder'], self.c['spinFileName'],
                                          'iso_sig2_{}'.format(self.isotope))
                self.del_z = del_z_init[1:]
                self.iso_sig2_z = iso_sig2_init[1:]
                Isf = interpolate.interp1d(input_year_iso, input_iso, self.c['int_type'],
            ...
            print(
                'No external file for surface isotope values found ({}), but you specified in the config file that '
                'isotope diffusion is on. The model will generate its own synthetic isotope data for you.'.format(
                    self.isotope))
