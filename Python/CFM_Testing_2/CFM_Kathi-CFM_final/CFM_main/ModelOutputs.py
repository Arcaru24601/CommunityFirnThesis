...
class ModelOutputs:
            ...
            # VV (23/03/2021)
                ...
                if self.MOgrid:  # gridding outputs
                    if varname == 'z':
                        ...
                        self.Mout_dict[varname] = np.zeros((TWlen + 1, len(self.grid_out) + 1),
                                                           dtype=self.c['output_bits'])

                    if varname == 'LWC':
                        self.Mout_dict[varname][0, :] = np.append(init_time,
                                                                  self.RGfun(MOd['z'], MOd[varname], self.grid_out))
                    elif varname != 'z':
    ...
    def updateMO(self, MOd, mtime, Wtracker):
        ...
        for varname in self.output_list:
            ...
            if self.MOgrid:
                if varname == 'LWC':
                    self.Mout_dict[varname][Wtracker, :] = np.append(mtime,
                                                                     self.RGfun(MOd['z'], MOd[varname], self.grid_out))
                elif ((varname == 'BCO') or (varname == 'DIP') or (varname == 'climate') or (varname == 'runoff') or (
                        varname == 'refreeze') or (varname == 'meltvol')):
                    self.Mout_dict[varname][Wtracker, :] = np.append(mtime, MOd[varname])
                elif varname == 'z':
                    continue
                else:
                    Ifun = interpolate.interp1d(MOd['z'], MOd[varname], kind=intkind, bounds_error=False,
                                                fill_value=np.nan)
