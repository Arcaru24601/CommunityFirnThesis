...
def bucket(self, iii):
    ...
    ### USER CHOICES ###
    try:
        ColeouLesaffre = self.c[
            'ColeouLesaffre']  # parameterising irreducible water content following Coléou and Lesaffre (1998) formulation [True/False]
        if ColeouLesaffre == False:
            IrrVal = self.c[
                'IrrVal']  # [%] irreducible water content: proportion of pore space that holds irreducible water
        RhoImp = self.c['RhoImp']  # density threshold for nodes to be considered as ice lens [kg m-3]
        DownToIce = self.c[
            'DownToIce']  # allows water to bypass all ice lenses until ice sheet is reached (depth where RhoImp density is definitely reached)
        if DownToIce == False:
            ThickImp = self.c[
                'ThickImp']  # thickness threshold for ice lens to be impermeable (all ice layers are impermeable if set to 0m) [m] # Using this is slow
        Ponding = self.c['Ponding']  # allowing LWC ponding above impermeable ice lenses [True/False]
        DirectRunoff = self.c[
            'DirectRunoff']  # (applicable if Ponding==True) fraction of excess LWC not considered for ponding but running off directly [between 0 and 1]
        RunoffZuoOerlemans = self.c[
            'RunoffZuoOerlemans']  # (applicable if Ponding==True) computing lateral runoff following Zuo and Oerlemans (1996) Eqs.(21,22) [True/False]
        Slope = self.c[
            'Slope']  # (used only if RunoffZuoOerlemans==True) slope value used in Zuo and Oerlemans (1996) Eq.(22) [/]
    ...
    liq_in_mass = max(melt_mass + (np.sum(self.LWC[0:ind1 + 1]) - pm_lwc) * RHO_W_KGM,
                      0)  # avoid negative lwcinput due to numerical round-off errors
    ...
    self.Tz = np.concatenate(
        ([pm_Tz], self.Tz[ind1 + 1:-1], self.Tz[-1] * np.ones(n_melted)))  # PM layer should have temp=T_MELT
    ...
    phivol_av = phivol * (
                RHO_I / RHO_W_KGM)  # saturated water content [m], i.e. tot. pot pore space avlbl for any LWC (Eq.9, Wever(2014); Discussion in Yamaguchi(2010))
    ilim = np.where(self.rho + phivol_av * RHO_W_KGM / self.dz > RHO_I)[
        0]  # nodes where saturation could lead to density>917
    ...
    if ColeouLesaffre:
        wmi = 0.057 * (
                    rhoi - self.rho) / self.rho + 0.017  # irred. water mass per mass of (water+firn) [/] (Coleou and Lesaffre (1998); Eq.3,Langen (2017))
        wmi[self.rho >= RhoImp] = 0.  # set 0 irreducible water in nodes exceeding impermeability threshold
        swi = np.zeros_like(wmi)
        imsk = self.rho < rhoi
        swi[imsk] = wmi[imsk] / (1 - wmi[imsk]) * rhoi * self.rho[imsk] / (RHO_W_KGM * (
                    rhoi - self.rho[imsk]))  # irreducible LWC per porosity space [/] (Eq.4 in Langen (2017))
    ...
    refr_cap0_supp = np.maximum(0,
                                refr_cap0 - self.LWC)  # refreezing capacity available for any liquid beyond what is currently present [m we]
    ...
    rho_pot = (
                          self.mass + refr_cap * RHO_W_KGM) / self.dz  # potential density after refreezing the maximum possible [kg m-3]
    ...
    phivol_av_pot = phivol_pot * (
                RHO_I / RHO_W_KGM)  # total pot. pore space avbl for storage aftr refreezing [m]; Eq.9, Wever(2014); Discussion: Yamaguchi(2010)
    ...
    if ColeouLesaffre:
        wmi_pot = 0.057 * (
                    rhoi - rho_pot) / rho_pot + 0.017  # irred. water mass per mass of (water+firn) [/] (Coleou and Lesaffre (1998); Eq.3,Langen (2017))
        ...
        imskp = rho_pot < rhoi
        swi_pot[imskp] = wmi_pot[imskp] / (1 - wmi_pot[imskp]) * rhoi * rho_pot[imskp] / (RHO_W_KGM * (
                    rhoi - rho_pot[imskp]))  # irreducible LWC per porosity space [/] (Eq.4 in Langen (2017))
    ...
    retcap_supp = np.maximum(0,
                             LWCirr_pot - LWCunf)  # retention capacity for additional LWC (assuming refreezing occurs first), potential minus how much is there already.
    ...
    if DownToIce:  # only ice sheet nodes are considered impermeable
        if np.any(self.rho < RhoImp):  # there is at least one node below density threshold
            imp = np.arange(np.where(self.rho < RhoImp)[0][-1] + 1, nnd, 1).astype(
                int)  # the nodes below last rho<RhoImp are impermeable
        ...
        if ThickImp > 0:
            lens0 = np.array([ii for ii in range(1, nnd) if
                              (self.rho[ii] >= RhoImp and self.rho[ii - 1] < RhoImp)])  # top index of each ice lens
            lens1 = np.array([ii for ii in range(0, nnd - 1) if
                              (self.rho[ii] >= RhoImp and self.rho[ii + 1] < RhoImp)])  # bottom index of each ice lens
            ...
            for ii in range(len(lens0)):
                ...
                if lensdz >= ThickImp or (ii == len(
                        lens0) - 1):  # impermeability if thickimp reached (bottom of domain is always impermeable)
    ...
    if len(imp) == 0:
        imp = [(
                           len(self.rho) - 1)]  # MS addition: If domain does not extend to full ice density, this will ensure the melt routine works (hack solution?)
    ...
    if liq_in_vol > 0:  # there is some liquid water input from the surface
        ...
        if ii0 >= imp[0]:  # impermeable barrier or not enough pore space prevents full distribution of meltinput
            ...
            storageinp = np.concatenate(
                (stcap[0:ii0 + 1], np.zeros(nnd - ii0 - 1)))  # each node above the barrier gets filled with its stcap
    ...
    if np.any(LWC_excess) > 0:  # if there is some excess LWC
        ...
        if ((np.any(stcap1[1:] > 0)) and (
                indb2 > jj0)):  # there is some storage capacity in the firn column, and it is deeper than jj0
            ...
            while ((jj0 <= indb1) or (tostore > 0)):
                if (np.where(stcap1[jj0:] > 0)[0]).size > 0:
                    ...
                    for jj2 in indsexc_deep:  # find underlying impermeable barrier for each node with some LWC_excess
                        jj1 = imp[np.where(imp >= jj2)[0][
                            0]] - 1  # jj1 becomes index of node above the impermeable barrier
                ...
                if imp[np.where(imp >= jj0)[0][0]] > jj1:  # jj0 and jj1 nodes not separated by an impermeable barrier
                    ...
                    tostore += sum(LWC_excess[
                                   jj0:jj1 + 1])  # all LWC_excess from jj0 to jj1 are subject to be blocked above the barrier
                    LWC1[jj0:jj1 + 1] = np.minimum(LWC1[jj0:jj1 + 1], LWCirr[jj0:jj1 + 1])  # LWC_excess is evacuated
                    LWCblocked[jj1] += tostore  # all LWC to be stored is blocked above the barrier
                    tostore = 0.  # tostore is set to 0
    ...
    if np.any(LWCblocked > 0):
        if Ponding == True:  # ponding is allowed
            ...
            rhofinal = self.rho
            phiempty = self.dz * (
                        rhoi - rhofinal) / RHO_W_KGM - self.LWC  # updte tot pot pore space avbl for LWC [m] (Eq.9; Wever(2014); Discussion in Yamaguchi(2010))
            ...
            for kk in np.flip(np.where(LWCblocked > 0)[0]):
                phiempty_cumf = np.cumsum(
                    np.flip(phiempty[0:kk + 1]))  # cumulative empty porespace until kk (inclusive), flipped

                if phiempty_cumf[-1] >= LWCblocked[kk]:  # enough porosity to accomodate ponding LWC
                    ifill = np.where(phiempty_cumf > LWCblocked[kk])[0][
                        0]  # [kk-ifill] is most upper node that accomodates LWCblocked[kk]
                ...
                #     LWCblocked[kk] = phiempty_cumf[kk] # MS added: need to also remove that volume from LWCblocked
                    ...
                    runofftot = runofftot + LWCblocked[kk] - phiempty_cumf[
                        kk]  # remove LWC that cannot be accomodated as runoff
                    LWCblocked[kk] = phiempty_cumf[kk]  # MS added: need to also remove that volume from LWCblocked
                ...
                if ifill == 0:  # The excess water can be contained in the kk node
                    ...
                    LWCfinal = self.LWC
                    LWCfinal[kk - ifill + 1:kk + 1] = LWCfinal[kk - ifill + 1:kk + 1] + phiempty[
                                                                                        kk - ifill + 1:kk + 1]  # fill nodes from kk-ifill (not included)
                    LWCblocked[kk] = LWCblocked[kk] - np.sum(
                        phiempty[kk - ifill + 1:kk + 1])  # remaining LWC in LWCblocked[kk]
                    phiempty[kk - ifill + 1:kk + 1] = 0.  # update phiempty
                    self.LWC[kk - ifill] = self.LWC[kk - ifill] + LWCblocked[
                        kk]  # node[kk-ifill] accomodates remaining of LWCblocked[kk]
    ...
    if RunoffZuoOerlemans == True:  # Calculations with post-refreezing values
        ...
        phivol = phi * self.dz  # pore space [m]
        phivol_av = phivol * (
                    RHO_I / RHO_W_KGM)  # total potential pore space available for refreezing [m] (Eq.9 in Wever (2014) and Discussion in Yamaguchi (2010))
        LWCirr = IrrVal * phivol_av  # maximum LWC that can be held as irreducible water [m]

        if ColeouLesaffre:
            wmi = 0.057 * (
                        rhoi - self.rho) / self.rho + 0.017  # irred. wtr mass per mass of (water+firn) [/] (Coleou and Lesaffre (1998); Eq.3 in Langen (2017))
            wmi[self.rho >= RhoImp] = 0.  # set 0 irreducible water in nodes exceeding impermeability threshold
            swi = np.zeros_like(wmi)
            imsk = self.rho < rhoi
            swi[imsk] = wmi[imsk] / (1 - wmi[imsk]) * rhoi * self.rho[imsk] / (RHO_W_KGM * (
                        rhoi - self.rho[imsk]))  # irreducible LWC per porosity space [/] (Eq.4, Langen(2017))
            LWCirr = phivol_av * swi  # maximum LWC that can be held as irreducible water [m]
...
def darcyscheme(self, iii):
    ...
    while timer < timetot:
        ...
        if timer == 0 or (n_mlt > 1 or self.rho[0] >= RhoImp or len(rho_lens0[rho_lens0 >= RhoImp]) != len(
                self.rho[self.rho >= RhoImp])):
            # Ice lens algorithm only at time step 0 or if the lens distribution has changed #
            lens0 = np.array([ii for ii in range(1, ncv) if
                              (self.rho[ii] >= RhoImp and self.rho[ii - 1] < RhoImp)])  # top index of each ice lens
            lens1 = np.array([ii for ii in range(0, ncv - 1) if
                              (self.rho[ii] >= RhoImp and self.rho[ii + 1] < RhoImp)])  # bottom index of each ice lens
            if self.rho[0] >= RhoImp:  # if surface node is an ice lens
                lens0 = np.append(0, lens0).astype(int)  # add to list
        ...
        theta_i = np.minimum(0.02, (
                    theta_s - 1e-9))  # irreducible water content, Yamaguchi 2010, limited to availabe porosity
        ...
        if i_inp == -1:  # impermeable lens at the surface
            ...
            liq_input1 = liq_input - np.cumsum(LWCacm)[
                i_inp - 1]  # liq water input remaining after filling nodes[0:i_inp]
            self.LWC[0:i_inp] = self.LWC[0:i_inp] + LWCacm[0:i_inp]  # update LWC of nodes[0:i_inp]
            self.LWC[i_inp] = self.LWC[i_inp] + min(liq_input1,
                                                    LWCacm[i_inp])  # node[i_inp] receives remaining liq_input1
        ...
        i00 = np.unique(np.concatenate((indsdry, imp, imp_d, satnofl_d, [ncv - 1]))).astype(
            int)  # add imp, imp_d, satnofl_d and lower boundary volume to volumes without outflow
        ...
        for j1 in np.flip(i11):  # find qlim at each interface in turn starting from bottom-most interface
            ...
            if timer == 0 or abs(glwflux_d1[j1] - abs(glwflux_d2[j1]) > 0.1 * glwflux_d1[j1]):
                ...
                glw[j1] = dtsub * glwflux_d1[
                    j1]  # glw set to flux corresponding to glw calculated at previous Darcy step
                glw[j1] = min(glw[j1], min(LWCav[j1], glwcacm[j1 + 1]))  # guess values cannot exceed available LWC
            ...
            if abs(f_eq > 1.):
                ...
                glw[j1] = flux_bisection(glw[j1], LWCav, glwcacm, theta_i[inds11], theta_s[inds11], glwc[inds11],
                                         self.dz[inds11], avG[inds11], nvG[inds11], mvG[inds11], eps_cvg)
            else:
                # Initial guess gives f_eq close to 0: Newton-Raphson algorithm #
                glw[j1] = flux_newtonraphson(glw[j1], LWCav, glwcacm, theta_i[inds11], theta_s[inds11], glwc[inds11],
                                             self.dz[inds11], avG[inds11], nvG[inds11], mvG[inds11], eps_cvg)
    ...
    ## Sanity checks
    if abs(sum(self.LWC) + refr_tot + runofftot - (
            melt_mass_tot / 1000 + sum(initial_lwc) + rain_vol_tot)) > 1e-12:  # check for water balance
        print('Liquid water loss/gain, amount:', sum(self.LWC) + sum(self.refrozen) + self.runoff - (
                    melt_mass_tot / 1000 + sum(initial_lwc) + rain_vol_tot))
