...
def firnConductivity(self, iii, K_ice):
    ...
    if self.c['conductivity'] == 'Calonne2019':  # Calonne et al. 2019
        ...
        K_firn = (1 - theta) * K_ice * K_air / (
                    kref_i * kref_a) * kref_snow + theta * K_ice / kref_i * kref_firn  # equation 5
...
def enthalpyDiff(self, iii):
    ...
    K_ice = 9.828 * np.exp(
        -0.0057 * self.Tz)  # thermal conductivity, ice (W/m/K), Cuffey and Paterson, eq. 9.2 (Yen 1981)
    ...
    c_vol = (
                        g_ice_1 * c_ice + g_liq_1 * c_liq) * tot_rho  # Voller eq. 10., the 'volume-averaged specific heat of mixture', or rho * cp. (so really heat capacity)

    K_firn = firnConductivity(self, iii, K_ice)  # thermal conductivity

    K_liq = K_water * (
                rho_liq_eff / 1000) ** 1.885  # I am assuming that conductivity of water in porous material follows a similar relationship to ice.
    K_eff = g_liq_1 * K_liq + g_ice_1 * K_firn  # effective conductivity

    phi_ret, g_liq, count, iterdiff = transient_solve_EN(z_edges_vec, z_P_vec, nt, self.dt[iii], K_eff, phi_0, nz_P,
                                                         nz_fv, phi_s, tot_rho, c_vol, self.LWC, self.mass, self.dz,
                                                         iii)
