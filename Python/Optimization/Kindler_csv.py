...
def csv_gen(temp,bdot):
    ...
    np.savetxt('CFM/CFM_main/CFMinput/Noise/Round6/'+str(int(temp)) + 'K/Acc.csv',Bdot_csv,delimiter=',')
    np.savetxt('CFM/CFM_main/CFMinput/Noise/Round6/'+str(int(temp)) + 'K/Temp.csv',Temp_csv,delimiter=',')

#Modes = np.array(['Temp','Acc','Both'])
#Rates = np.array([50,200,500,1000])

for i in range(len(Input_temp)):
    path = Path('CFM/CFM_main/CFMinput/Noise/Round6/' + str(int(Input_temp[i])) + 'K')
