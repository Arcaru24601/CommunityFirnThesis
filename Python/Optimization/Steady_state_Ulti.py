...
spin_year = 1000
model_year = 500
spin_year = spin_year + model_year/2
...
def func(temp,N_ref,var_dict,rhos):
    ...
    subprocess.run('python main.py FirnAir_Noise_Ulti.json -n')
...
def Data_crunch_Ulti(Model,Indices,rho_surface_uncertainty_Flag,diff_param_Flag):
    print(os.getcwd())
    ...
    for j,val in enumerate(Indices):
        mu_d15n = df[str(Model)][j]
        d15N_dist = np.random.normal(np.asarray(mu_d15n),0.02,size=600)
        ...
        for k in range(len(d15N_dist)):
            ...
            print('Target temp', Input_temp[j])
            ...
            if rho_surface_uncertainty_Flag == True:
                rho_s_distribution = np.random.normal(330,20,size=600)
                file = open('CFM/CFM_main/FirnAir_Noise_Ulti.json')
                data = json.load(file)
            ...
            if diff_param_Flag == True:
                ...
                diff_param = np.random.choice(diff,1)
            ...
            # =============================================================================
            i_acc = np.full(len(Time),Input_acc[j])
            ...
            folder_path = 'resultsFolder/Ulti/' + str(Model) + '/' + str(Indices[j]) 
...
Data_crunch_Ulti('HLD',Temp_input,True,True)
