...
TestN = np.zeros((H,4))
CoD_T = np.zeros((H,4))
rfolder = 'CFM/CFM_main/CFMoutput/Noise/surf/'
...
df.to_csv('resultsFolder/out_model2.csv')
df2.to_csv('resultsFolder/out_diffu2.csv')


from Kindler_fit_ODR import input_file,expfunc,rho_0
...
for (index, column) in enumerate(df):
    ...
    ax1.plot(rho_s,d15N,label=str(df.columns[index]),color=cmap(cmap_intervals[index]))
    ax1.plot(rho_s,d15N,linestyle[index],fillstyle='none',color=cmap(cmap_intervals[index]))
...
plt.savefig('Noise/Noise_surf.png',dpi=300)
