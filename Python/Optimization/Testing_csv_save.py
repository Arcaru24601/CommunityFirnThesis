# -*- coding: utf-8 -*-
"""
Created on Thu Mar 30 15:10:34 2023
...
Dist_num =  ['Dist' + str(i) for i in np.arange(2)]### Regime number
Point_dist_num = np.arange(50)


def root_find(path_to_result,N_ref,j,j2):
    count = 0
    ...
    max_int = np.shape(var_dict['count'])[0]
    print(res_c[1])
    for g,key in enumerate(var_dict):
        save_array[g] = var_dict[key][max_int-1]
        print(g,key)
        print('Test vardict')
    save_array[4] = N_ref
    print('Test N ref')
    save_array[5] = Input_temp[j2]
    print('Test Temp')
    print(save_array)
    return save_array    
...
headerList = ['count', 'd15N model', 'temp model', 'cost func', 'd15N ref', ' temp ref']
model_path = '../CFM/CFM_main/CFMoutput/OptiNoise/CFMresults.hdf5.' ### Insert name
#results_path = 'resultsFolder/minimizer.h5' #Add name
folder_path1 = 'resultsFolder/TestingNow/'
def Data_crunch(Model,Dist):
    for i in range(len(Model)):
        ...
        for j in range(len(Dist)):
            ...
            Csv_point = df[str(Modela[j])]
            s = np.random.normal(Csv_point[j2],0.02,size=2000)
            d15N_dist = s[(abs(s - s.mean())) < (3 * s.std())][:2]

            Array = np.zeros((len(d15N_dist),6))
            ...
            for k in range(len(d15N_dist)):
                ...
                folder_path = folder_path1 + str(Model[i]) + '/' + str(Dists2[j]) 
                path = Path(folder_path)
                path.mkdir(parents=True, exist_ok=True)
                results_path = folder_path + '/' + 'Point'  + str(k) + '.h5'
                try:
                    Array[k,:] = root_find(results_path,d15N_ref,k,j2)
                    
                except Exception as e: print(e)
            print(Array)
            df_array = pd.DataFrame(Array)
            df_array.to_csv(folder_path1 + str(Model[i]) + '/' + str(Dists2[j]) + '.csv', header=headerList, index=False)
            np.savetxt(folder_path1 + str(Model[i]) + '/' + str(Dists2[j]) + '.csv',Array,delimiter=",")
    return results_path
