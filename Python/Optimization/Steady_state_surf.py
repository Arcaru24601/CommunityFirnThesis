...
results_path = 'resultsFolder/Surf3/minimizer.h5' #Add name
...
def func(temp,N_ref,var_dict,j):
    ...
    np.savetxt(r'D:/GitHub/CommunityFirnThesis/CommunityFirnThesis/Python/CFM/CFM_main/CFMinput/OptiNoise/optimize_temp.csv', input_temp, delimiter=',')
    
    
    os.chdir('../CFM/CFM_main')
    file = open('FirnAir_Noise_surf.json')
    data = json.load(file)
    data['rhos0'] = rho_0(temp,Input_acc[Indices_d15N[j]])
        
    with open("FirnAir_Noise_surf.json", 'w') as f:
        json.dump(data, f,indent = 2)
        
        # Closing file
    f.close()
    #np.savetxt('CFMinput/OptiNoise/optimize_acc.csv', input_acc, delimiter=',')
    subprocess.run('python main.py FirnAir_Noise_surf.json -n')
    ...
    if os.path.exists(model_path):
        ...
        print('Model rho',rho_0(temp,Acc))
...
def root_find(path_to_result,N_ref,j):
    count = 0
    ...
    res_c = brentq(func,a = 215,b = 250,args=(N_ref,var_dict,j),full_output = True,xtol=2e-3,rtol=8.88e-6)
    entry_0 = np.where(var_dict['count'] == 0)[0]
    var_dict['count'] = np.delete(var_dict['count'], entry_0[1:])
    var_dict['count'] = var_dict['count'][:-1]
    max_int = np.shape(var_dict['count'])[0]
...
folder_path1 = 'resultsFolder/Surf3/'
...
def Data_crunch_Test(Model):
    for i in range(len(Model)):
        os.chdir('../')
        file = open('CFM/CFM_main/FirnAir_Noise_surf.json')
        data = json.load(file)
        data['physRho'] = str(Model[i])
            
        with open("CFM/CFM_main/FirnAir_Noise_surf.json", 'w') as f:
            json.dump(data, f,indent = 2)
        ...
        for j in range(len(Indices_d15N)):
            ...
            d15N_dist = s[(abs(s - s.mean())) < (3 * s.std())][:100]
            ...
            for k in range(len(d15N_dist)):
                ...
                #print('Original d15N', mu_d15n)
                print('Target rhos', rho_0(Input_temp[Indices_d15N[j]],Input_acc[Indices_d15N[j]]))
            ...
            #D:\GitHub\CommunityFirnThesis\CommunityFirnThesis\Python\CFM\CFM_main\CFMinput\OptiNoise
                ...
                folder_path = folder_path1 + str(Model[i]) + '/' + str(Indices_d15N[j]) 
                path = Path(folder_path)
                path.mkdir(parents=True, exist_ok=True)
                results_path = folder_path + '/' + 'Point'  + str(k) + '.h5'
                try:
                    root_find(results_path,d15N_ref,j)
...
T,d15N_dist = Data_crunch_Test(Model_n)
