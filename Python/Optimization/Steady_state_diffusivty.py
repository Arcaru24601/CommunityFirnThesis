...
results_path = 'resultsFolder/Diffu/minimizer.h5' #Add name
...
def func(temp,N_ref,var_dict):
    ...
    if os.path.exists(model_path):
        ...
        var_dict['temp'][count] = temp
        #print(temp,temperature_model,'Testing')
        rho_co = rho_bco(temp)
...
df = pd.read_csv('resultsFolder/diff_out_diffu.csv',sep=',')

model_path = '../CFM/CFM_main/CFMoutput/OptiNoise/CFMresults.hdf5' ### Insert name
#results_path = 'resultsFolder/minimizer.h5' #Add name
folder_path1 = 'resultsFolder/Diffu/'
...
def Data_crunch_Test(Model):
    for i in range(len(Model)):
        os.chdir('../')
        file = open('CFM/CFM_main/Air_OptiNoise_diffu.json')
        data = json.load(file)
        data['Diffu_param'] = str(Model[i])
            
        with open("CFM/CFM_main/Air_OptiNoise_diffu.json", 'w') as f:
            json.dump(data, f,indent = 2)
...
T,d15N_dist = Data_crunch_Test(Diffus)
