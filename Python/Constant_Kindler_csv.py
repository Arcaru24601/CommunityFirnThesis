# -*- coding: utf-8 -*-
"""
Created on Thu Feb  9 13:50:19 2023
...
def Run(Model,site):
    ...
    data['resultsFolder'] = 'CFMoutput/Constant_Forcing_Kindler/' + str(site) + '/' + str(Model)
    data['InputFileFolder'] = 'CFMinput/Constant_Forcing_Kindler'
    if site == 'NGRIP':
        data['rhos0'] = 300
    elif site == 'NEEM':
        data['rhos0'] = 340
...
def csv_gen(site):
    if site == 'NGRIP':    
        ...
        Temp = np.full(len(Time),241.45)
        Bdot = np.full(len(Time),1.9e-1)
    elif site == 'NEEM':    
        Time = np.array([250,1000,5000])
        #Time = np.array([1,3000,5000])
        Temp = np.full(len(Time),244.15)
        Bdot = np.full(len(Time),2.16e-1)
        ...
        Time = np.array([250,1000,5000])
        Temp = np.full(len(Time),218.65)
        Bdot = np.full(len(Time),2.7e-2)
    elif site == 'South':
        Time = np.array([250,1000,5000])
        Temp = np.full(len(Time),223.75)
        Bdot = np.full(len(Time),7.6e-2)
    #plt.plot(Time,Temp)
    #plt.plot(Time,Bdot)
    print(Bdot)
    ...
    np.savetxt('CFM/CFM_main/CFMinput/Constant_Forcing_Kindler/'+str(site)+'_acc.csv',Bdot_csv,delimiter=',')
    np.savetxt('CFM/CFM_main/CFMinput/Constant_Forcing_Kindler/'+str(site)+'_Temp.csv',Temp_csv,delimiter=',')
...
for Site in Sites:
    path = Path('CFM/CFM_main/CFMinput/Constant_Forcing_Kindler/' + Site)
