...
class CoD_plotter():
    ...
    def plotting(self):
        ...
        Times = [2000,4000,5000,6000,8000,9900]
        cmap_interval = np.linspace(0,1,len(Times))
        time_labels = ['$t_0$', '$t_1$', '$t_2$', '$t_3$', '$t_4$', '$t_5$', '$t_6$']
        
        
        self.ax02 = self.ax[0,0].twinx()
        self.ax[0,0].set_ylabel("Temperature Forcing [K]",color=cmap(cmap_interval[-1]),fontsize=labelFont)
        self.ax02.set_ylabel("Accumulation Forcing [$\mathrm{my}^{-1}$ ice eq.]",color=cmap(cmap_interval[3]),fontsize=labelFont)
        ...
        self.ax[0,0].plot(self.climate[:,0],self.climate[:,2],color=cmap(cmap_interval[-1]))
        self.ax02.plot(self.climate[:,0],self.climate[:,1],color=cmap(cmap_interval[3]))
